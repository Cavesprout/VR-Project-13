...
public class QuizBoardController : MonoBehaviour
{
    ...
    QuizData LoadExampleQuiz()
    {
        ...
        QuestionAnswers.Add("Choice 0");
        QuestionAnswers.Add("Choice 1");
        QuestionAnswers.Add("Choice 2");
        QuestionAnswers.Add("Choice 3");
        Answers.Add(QuestionAnswers);
        QuestionAnswers = new List<string>();
    ...
    public void ShowError(string message)
    {
        ...
        ErrorTXT.GetComponent<TMPro.TextMeshProUGUI>().text = "This Should Display an Error";
    }
    ...
    public void PickChoice(int questionNum)
    {
        if (selectedAnswer != -1)
        {
            answerPicker[selectedAnswer].GetComponent<Material>().SetColor("_Color", Color.green);
        }
        answerPicker[questionNum].GetComponent<Material>().SetColor("_Color", Color.green);
        selectedAnswer = questionNum;
    }
    ...
    public void SubmitAnswer()
    {
        ...
        {
            ShowError("Wrong Answer Selected");
        }
        answerPicker[selectedAnswer].GetComponent<Material>().SetColor("_Color", Color.green);
        selectedAnswer = -1;
    }
    ...
    public void StartQuiz()
    {
        ...
        {
            activeQuestion = 0;
            titleTXT.GetComponent<TMPro.TextMeshProUGUI>().text = ActiveQuiz.Questions[activeQuestion];
            for (int i = 0; i < ActiveQuiz.Answers[activeQuestion].Count; i++)
            {
                answerTXT[i].GetComponent<TMPro.TextMeshProUGUI>().text = ActiveQuiz.Answers[activeQuestion][i];
            }
            questionTXT.GetComponent<TMPro.TextMeshProUGUI>().text = "This Should Display an Error";
        }
        else
        {
